...
public sealed class PostgresGenerator : IIncrementalGenerator
{
    ...
    {
        RegisterExtensionMethods(context);
        RegisterInterceptorsForQueries(context);
        RegisterInterceptorsForExecutes(context);
    }

    private void RegisterExtensionMethods(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterPostInitializationOutput(ctx =>
        {
            ctx.AddSource($"SqueelExtensions.g.cs", SourceText.From($$"""
                {{GeneratedFileOptions.Header}}

                namespace Squeel;

                {{GeneratedFileOptions.Attribute}}
                internal static class SqueelExtensions
                {
                    {{GeneratedFileOptions.Attribute}}
    ...
    private void RegisterInterceptorsForQueries(IncrementalGeneratorInitializationContext context)
    {
    ...
    private void RegisterInterceptorsForExecutes(IncrementalGeneratorInitializationContext context)
    {
        context.RegisterSourceOutput(context.SyntaxProvider
            .CreateSyntaxProvider(Filter, Transform)
            .Where(info =>
    ...
    public static string ToParameterizedString(InterpolatedStringExpressionSyntax interpolated, out Dictionary<string, IdentifierNameSyntax> parameters)
    {
    ...
    public static object ToExampleValue(TypeInfo typeInfo)
    {
